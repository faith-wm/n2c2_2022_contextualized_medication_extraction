...
def loadData(df):
    ...
    for i in range(len(df)):
        data.append({'index': i,
                     'sentence':df['sentence'][i],   #columns created using the pre-processing script
...
def preprocessingForBert(data,tokenizer,max_length=510):
...
def training(train_df,dev_df,type,epochs,model_name,tokenizer,learning_rate,warmup_steps=0,batch_size=2):
    ...
    for i in range(epochs):
        ...
        for step, batch in enumerate(dataloader_train):
            ...
            model.zero_grad()
            outputs = model(b_input_ids, token_type_ids=b_token_type_id, attention_mask=b_attention_mask,labels=b_labels)
        ...
        for batch in dataloader_val:
            ...
            b_input_ids, b_token_type_id, b_attention_mask, b_labels = batch
            with torch.no_grad():
                outputs = model(b_input_ids, token_type_ids=b_token_type_id,attention_mask=b_attention_mask)
            logits = outputs[0]
        ...
        #columns sentence, label,  file_name, ann_type, medication, start_span, end_span
        tag_values = ['Disposition', 'NoDisposition', 'Undetermined']
...
def bioClinicalBert():
    ...
    print('\n\n{:*^50}'.format('TRAINING FOR MODEL '+model_name))
    training(epochs=10,learning_rate=2e-5,train_df=train_df,dev_df=dev_df,type='window200',model_name=model_name,tokenizer=tokenizer,batch_size=20)
